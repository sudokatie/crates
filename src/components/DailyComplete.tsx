...
export function DailyComplete({
  ...
  const shareCode = generateShareCode(todayString(), totalMoves, levelsCompleted);
      ...
      <div className="bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="bg-gray-700 rounded p-3 text-center">
            <div className="text-2xl font-bold text-white">{totalMoves}</div>
            <div className="text-xs text-gray-400">Total Moves</div>
          </div>
          <div className="bg-gray-700 rounded p-3 text-center">
            <div className="text-2xl font-bold text-white">{totalPushes}</div>
        ...
        {!submitted ? (
          <div className="mb-4">
            <input
              ...
              maxLength={20}
              className="w-full px-4 py-2 bg-gray-700 text-white rounded border border-gray-600 focus:border-amber-500 focus:outline-none mb-2"
            ...
            <button
              ...
              disabled={!name.trim()}
              className="w-full py-2 bg-amber-600 hover:bg-amber-500 disabled:bg-gray-600 disabled:cursor-not-allowed text-white font-bold rounded transition-colors"
            >
              Submit Score
            </button>
          </div>
        ...
        <div className="mb-4">
          ...
          <div className="flex gap-2">
            <code className="flex-1 px-3 py-2 bg-gray-900 text-amber-400 rounded font-mono text-sm">
              {shareCode}
            </code>
            <button
              onClick={handleCopy}
            ...
            <div className="bg-gray-900 rounded p-2 max-h-40 overflow-y-auto">
              {leaderboard.slice(0, 5).map((entry, i) => (
                ...
                >
                  ...
                  <span className="text-amber-400 ml-2">{entry.totalMoves}m</span>
                </div>
              ))}
            </div>
          </div>
